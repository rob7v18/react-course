...
import { getMoviesByName } from "./utils/helpers";
import './App.css';
...
function App() {
  ...
  useEffect(() => {
    ...
    getMoviesByName(searchTerm).then((movies) => {
  ...
  return (
   <div>
    ...
    {movies.map((movie) => {
      return (<MovieCard key={movie.imdbID} onImageClick={handleClick} title={movie.Title} type={movie.Type} posterUrl={movie.Poster} />)
    })}
